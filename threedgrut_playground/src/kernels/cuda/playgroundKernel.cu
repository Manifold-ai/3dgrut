...
constexpr uint32_t MAX_BOUNCES =
    32; // Maximum number of mirror material bounces only (irrelevant to pbr)
constexpr uint32_t TIMEOUT_ITERATIONS =
    1000; // Terminate ray after max iterations to avoid infinite loop
constexpr float REFRACTION_EPS_SHIFT =
    1e-5; // Add eps amount to refracted rays pos to avoid repeated collisions
...
extern "C" __global__ void __raygen__rg() {
  ...
  // 3. PBR Materials: pathThroughput is zero -> no more color is reflected
  // back, or
  // 4. Mirrors: No remaining bounces
  while (!payload.rayMissed && (length(payload.pathThroughput) > 0.0001) &&
    ...
    // First trace the closest hit point with a mesh, and compute new ray
    // direction + how much light scattered
    payload.bsdfValue = make_float3(
        1.0f); // Will contain how much light is scattered along this path
    payload.nextEmissive =
        make_float3(0.0f); // Will contain how much additional light is emitted
                           // along this path
    ...
    float4 volumetricRadDns =
        traceGaussians(rayData, rayOri, rayDir, 1e-9, next_ray_t, &payload);
    float3 radiance =
    ...
    // transmittance, maybe bookkeep directLight and pathThroughput at mesh
    // intersection points and backtrack?

    // Aggregate volumetric radiance
    payload.accumulatedAlpha += density * (1.0f - payload.accumulatedAlpha);
    payload.accumulatedColor +=
        payload.pathThroughput *
        radiance; // Contribution of Gaussian radiance to path
    ...
    payload.pathThroughput *=
        (1.0f - density); // Modulate throughput by volumetric transmittance

    // Update attenuation of light according to BRDF + BTDF
    // This is how much light was emitted by the surface + scattered along this
    // path, modulated by the throughput
    payload.accumulatedColor +=
        payload.pathThroughput * (payload.directLight + payload.nextEmissive);
    payload.pathThroughput *= payload.bsdfValue;

    // Keep this in case additional background sampling is done external to the
    // shader
    lastRayOri = rayOri;
    lastRayDir = rayDir;
  ...
  const float4 rgba =
      make_float4(payload.accumulatedColor.x, payload.accumulatedColor.y,
                  payload.accumulatedColor.z, payload.accumulatedAlpha);
...
static __device__ __inline__ bool refract(float3 &out_dir, const float3 ray_d,
  ...
  if (can_refract) {
    ...
    float3 r_out_parallel =
        -sqrt(fabsf(1.0 - dot(r_out_perp, r_out_perp))) * normal;
...
static __device__ __inline__ void handleMirror(const float3 ray_d,
                                               float3 normal,
                                               float3 &new_ray_dir,
...
handleGlass(const float3 ray_d, float3 normal, float3 &new_ray_dir,
  ...
  float ior = n2 / n1;
  bool is_refracted = refract(new_ray_dir, ray_d, normal, ior,
                              rndSeed); // Updates new_ray_dir if refracted
  if (!is_refracted) {
    // Perfect reflection, e.g: ray_d - 2.0 * dot(ray_d, normal) * normal
    float3 reflected_normal = dot(ray_d, normal) < 0.0 ? normal : -normal;
    new_ray_dir = -reflect(ray_d, reflected_normal);
    ...
    // Move next ray origin a bit forward to avoid repetitive collisions with
    // the same face
...
static __device__ __inline__ void
handlePBR(const float3 ray_o, const float3 ray_d, float3 normal,
          unsigned int &numBounces, float &hit_t, float3 &new_ray_dir,
          unsigned int &rndSeed, HybridRayPayload *payload) {
  sampled_cook_torrance_brdf(ray_o, ray_d, hit_t, normal, new_ray_dir, rndSeed,
                             payload);
  new_ray_dir = new_ray_dir / length(new_ray_dir);
}

static __device__ __inline__ void handleDiffuse(const float3 ray_o,
                                                const float3 ray_d,
                                                float3 normal, float &hit_t,
                                                HybridRayPayload *payload) {
  // Accumulate all Gaussian particles up to intersection with mesh surface
  // first
  const float4 volumetricRadDns =
      traceGaussians(*(payload->rayData), ray_o, ray_d, 1e-9, hit_t, payload);
  const float3 volRadiance =
      make_float3(volumetricRadDns.x, volumetricRadDns.y, volumetricRadDns.z);
...
static __device__ __inline__ float3 getSmoothNormal() {
  ...
  const float3 n0 =
      make_float3(params.vNormals[v0_idx][0], params.vNormals[v0_idx][1],
                  params.vNormals[v0_idx][2]);
  const float3 n1 =
      make_float3(params.vNormals[v1_idx][0], params.vNormals[v1_idx][1],
                  params.vNormals[v1_idx][2]);
  const float3 n2 =
      make_float3(params.vNormals[v2_idx][0], params.vNormals[v2_idx][1],
                  params.vNormals[v2_idx][2]);
  const float2 barycentric = optixGetTriangleBarycentrics();
  float3 interpolated_normal = (1 - barycentric.x - barycentric.y) * n0 +
...
extern "C" __global__ void __closesthit__ch() {
  ...
  // e.g. guaranteed that:
  // unsigned int next_render_pass = getNextTraceState(); next_render_pass ==
  // PGRNDTracePrimitivesPass;

  // Read inputs off payload
  HybridRayPayload *payload = getRayPayload();
  unsigned int numBounces =
      payload->numBounces; // Number of times ray was reflected so far
  ...
  const float3 ray_o =
      optixGetWorldRayOrigin(); // Ray origin, when ray intersected the surface
  const float3 ray_d = optixGetWorldRayDirection(); // Ray direction, when ray
                                                    // intersected the surface
  // Compute normals using interplated precomputed vertex normals or directly
  // from vertex positions ("non-smooth")
  const float3 normal = (params.playgroundOpts & PGRNDRenderSmoothNormals)
                            ? getSmoothNormal()
                            : getHardNormal();
  ...
  unsigned int next_render_pass =
      PGRNDTraceRTGaussiansPass; // Next render pass in most cases is volumetric
                                 // tracing

  // The following vars will be initialized with new data: new_ray_dir,
  // numBounces, hit_t, rndSeed
  ...
  // Output: Ray hit something so it is considered redirected (->Gaussians
  // pass), or terminate
...
extern "C" __global__ void __miss__ms() {
  // Ray missed: no primitives - trace remaining Gaussians till bbox end, or
  // terminate
